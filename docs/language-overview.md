...
TEMPLATE DATA TASK APPLIES_TO ALL ANY WRITES READS METHOD WHERE KEY
...
TIMEOUT RETRY PURE BACKOFF TTL VARY WITH PARALLEL RECEIVE ON EVENT
...
- INSERT/UPSERT/UPDATE/DELETE/FANOUT requires `CAPABILITY write`.
...
- Every UPSERT key must exactly match the primary key or a declared UNIQUE index.

**Atomicity checking:**
- IDEMPOTENCY is limited to mutating SQL flows whose sources share one dialect and one physical database.
- Inside an idempotent transaction, service calls must be `PURE` or provider-deduplicated with the exact flow key. TRY and UPLOAD are rejected; EFFECT uses the transactional outbox.
