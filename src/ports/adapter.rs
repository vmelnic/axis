...
pub trait SourceAdapter: Send + Sync {
    ...
    fn insert(&self, _req: InsertRequest) -> BoxFut<'_, Result<Value, AdapterError>> {
        ...
        Box::pin(async move {
            Err(AdapterError::Unsupported(format!(
                "INSERT not supported for {stype}"
            )))
        })
    }
    ...
    fn update(&self, _req: UpdateRequest) -> BoxFut<'_, Result<u64, AdapterError>> {
        ...
        Box::pin(async move {
            Err(AdapterError::Unsupported(format!(
                "UPDATE not supported for {stype}"
            )))
        })
    }
    ...
    fn delete(&self, _req: DeleteRequest) -> BoxFut<'_, Result<u64, AdapterError>> {
        ...
        Box::pin(async move {
            Err(AdapterError::Unsupported(format!(
                "DELETE not supported for {stype}"
            )))
        })
    }
}
...
pub type AdapterFactory = Box<
    dyn Fn(
        ...
        ) -> BoxFut<'static, Result<Box<dyn SourceAdapter>, AdapterError>>
        + Send
        + Sync,
>;
    ...
    async fn test_adapter_registry() {
        ...
        registry.register(
            ...
            Box::new(|_config: &HashMap<String, String>| {
                Box::pin(async move { Ok(Box::new(MockRedis::new()) as Box<dyn SourceAdapter>) })
            }),
        );
